...
health_url = "https://raw.githubusercontent.com/niloufar07/RomeTerminals/main/170424_Roma_PuntiInteresse.csv"
rome_url = "https://raw.githubusercontent.com/niloufar07/RomeTerminals/main/okrome.csv"
...
# Load datasets
@st.cache  # Cache the datasets to avoid fetching multiple times
def load_data():
    health = fetch_data(health_url)
    rome = fetch_data(rome_url)
    return health, rome
...
# Asynchronous function to get coordinates with retry mechanism
async def get_coordinates_async(session, station):
    if station in station_coordinates:
        return station_coordinates[station]
    
    url = f"https://nominatim.openstreetmap.org/search?q={station}, Rome, Italy&format=json&limit=1"
...
async def fetch_coordinates(stations):
    async with aiohttp.ClientSession() as session:
        tasks = [get_coordinates_async(session, station) for station in stations]
        results = await asyncio.gather(*tasks)
        return {station: coord for station, coord in zip(stations, results)}

def main():
    # Load datasets
    health, rome = load_data()

    # Fetch coordinates asynchronously
    station_coordinates.update(asyncio.run(fetch_coordinates(stations)))

    # Print station coordinates (for verification)
    ...
    for station_name, (station_lat, station_lon) in station_coordinates.items():
        ...
        folium.Marker(location=[nearest_hospital_lat, nearest_hospital_lon], popup=popup_content,
